...
`tools/list` at `https://gateway.pipeworx.io/ifsc-in/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/ifsc_lookup \
  -H 'Content-Type: application/json' \
  -d '{"ifsc":"HDFC0CAGSBK"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/ifsc_lookup`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
